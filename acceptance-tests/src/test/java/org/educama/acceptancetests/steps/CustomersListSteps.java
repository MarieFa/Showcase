...
public class CustomersListSteps {
    ...
    @Step
    public void addNewCustomer(String name, boolean save) {
        customersListPage.openCustomerCapturePage();
        customerCapturePage.createCustomer(name, save);
    }
    ...
    @Step
    public void openAppleChangeView() {
        List<String> customerList;
        String name = "Apple";
        String id = "";
            List<String> customerListName = customersListPage.getCustomersList(1);
            for (int i = 0; i < customerListName.size(); i++) {
                if (customerListName.get(i).equals(name)) {
                id = customersListPage.getCustomersList(0).get(i);
                }
            }
            if (id != "") {
            customersListPage.openCustomerChangePage(id);
            }
    }
    @Step
    public void changeCustomerData(boolean save) {
        customersListPage.changeCustomerName(save);
    }
    @Step
